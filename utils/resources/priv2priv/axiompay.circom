...
template VerifyPrivateTransfer() {
    ...
    component check2 = CheckEncryptResult();
    check2.p <== pb;
    check2.vk <== cbAmountK;
    check2.m <== amount;
    check2.c1y <== c1byAmount;
    check2.c2y <== c2byAmount;

    // check balance must greater or equal than amount
    component check3 = GreaterEqThan(40);
    check3.in[0] <== balance;
    check3.in[1] <== amount;

    component check4 = CheckBalanceMatch();
    check4.balance <== balance;
    check4.privateKey <== privateKey;
    check4.c1Balance <== c1balance;
    check4.c2Balance <== c2balance;
}
...
component main { public [c1ayAmount,c1byAmount,c2ayAmount,c2byAmount,c1balance,c2balance] } = VerifyPrivateTransfer();
